...
{
    public class CodeScriptCommands : BaseCommandModule
    {
        private ConfigJson configJson = ConfigJson.GetJSonAsync().Result;
        private DiscordChannel GetCodeChannel(CommandContext ctx) => ctx.Channel.Guild.GetChannel(configJson.CodeChan);

        [Command("cs")]
        [Description("put C# code in coding channel and display a link to that code.")]
        public async Task CSCode(CommandContext ctx, string headline, params string[] msg)
        {
        ...
        public async Task PYCode(CommandContext ctx, string headline, params string[] msg)
        {
            await ctx.Client.SendMessageAsync(GetCodeChannel(ctx), "", false, Embed(ctx, "Python")).ConfigureAwait(false);
            var newMessage =
                await ctx.Client.SendMessageAsync(GetCodeChannel(ctx), BuildMessage(ctx, configJson.CsPrefix, headline)).ConfigureAwait(false);
            await ctx.Channel.SendMessageAsync("", false, Embed(ctx, newMessage, headline)).ConfigureAwait(false);
            await ctx.Message.DeleteAsync().ConfigureAwait(false);
